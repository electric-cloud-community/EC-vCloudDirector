...
# vclouddirector.cgi -
#
# Get/set EC-vCloudDirector configuration info for UI
#
...
################################
# getCfgList - Return the list of configurations from vCloudDirector
#
# Arguments:
#   -
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/vclouddirector_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $url = $gcfg->getCol("$cfg/vclouddirector_url");
        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <service_url>" . xmlQuote($url) . "</service_url>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";
